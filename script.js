...
function createStartMenu() {
    ...
    if (startButtonCheck == true) {
        ...
        allAppsButton.onclick = createAllAppsBox;
        ...
        startMenuBox.appendChild(AppsBox);
...
let allAppsBoxCheck = true;
function createAllAppsBox() {
    event.stopPropagation();
    let startMenuBox = document.querySelector('.start-menu-box');
    ...
    const allAppsButton = document.createElement('div');
    allAppsButton.classList.add('start-menu-all-app-button');
    allAppsButton.textContent = 'Back';
    allAppsButton.onclick = createAllAppsBox;
    ...
    if (allAppsBoxCheck == true) {
        ...
        let appCategories = ['Apps', 'Games', 'Tools'];

        let allAppsContainer = document.createElement('div');
        allAppsContainer.classList.add('all-apps-conatiner');

        for (let i = 0; i < appCategories.length; i++) {
            let appCategoryTitle = document.createElement('div');
            appCategoryTitle.classList.add('app-category-title');
            appCategoryTitle.textContent = appCategories[i];

            let allAppsBox = document.createElement('div');
            allAppsBox.classList.add('all-apps-box');

            for (let j = 0; j < appNameList[i].length; j++) {
                let appBox = document.createElement('div');
                appBox.classList.add('start-menu-app-box');
                appBox.onclick = function () {
                    createApplication(appNameList[i][j], appUrlList[i][j]);
                };

                let appIcon = document.createElement('img');
                appIcon.classList.add('start-menu-app-icon');
                appIcon.setAttribute('src', appIconList[i][j]);
                appIcon.setAttribute('alt', appNameList[i][j]);

                let appName = document.createElement('div');
                appName.classList.add('start-menu-app-name');
                appName.textContent = appNameList[i][j];
            ...
            allAppsContainer.appendChild(appCategoryTitle);
            allAppsContainer.appendChild(allAppsBox);
        }

        // Append allAppsContainer to the parent element
        startMenuBox.appendChild(searchBarBox);
        startMenuBox.appendChild(buttonBox);
        startMenuBox.appendChild(allAppsContainer);

        allAppsBoxCheck = false;
    }
    else {
        let appNameList = [
            'File Explorer',
...
function createQuickSettingMenu() {
    ...
    if (quickSettingMenuCheck == true) {
        ...
        const toggleButtonsData = [
            ...
            { iconSrc: 'images/bluetooth_icon.png', name: 'Bluetooth', moreOption: true },
            { iconSrc: 'images/battery_saver_icon.svg', name: 'Battery saver' },
            { iconSrc: 'images/brightness_icon.png', name: 'Nigth light' },
            { iconSrc: 'images/accessibility_icon.png', name: 'Accessibility' },
            { iconSrc: 'images/screen_cast_icon.png', name: 'Cast' },
            { iconSrc: 'images/mobile_hostpot_icon.png', name: 'Mobile hotspot' },
            { iconSrc: 'images/project_screen_icon.png', name: 'Project' },
            { iconSrc: 'images/nearby_sharing.png', name: 'Nearby sharing' }
        ];
        ...
        brightnessIcon.alt = '';
        brightnessIcon.classList.add('brightness-icon');
        ...
        volumeIcon.src = 'images/speaker_icon.png';
        volumeIcon.alt = '';
...
function createHiddenIconsBox() {
    ...
    if (hiddenIconBoxCheck == true) {
        ...
        const imageSources = [
            "images/power_automate_icon.png",
            "images/windows_defender_icon.png",
            "images/microsoft_team.png",
            "images/onedrive_icon.png"
        ];
...
function createContextMenuBox(event) {
    ...
    if (contextMenuBoxCheck == true) {
        ...
        const contextMenuData = [
            ...
            { iconSrc: 'images/add_new_folder_icon.png', name: 'New' },
            { iconSrc: 'images/display_setting_icon.png', name: 'Display settings' },
            { iconSrc: 'images/personalize_icon.png', name: 'Personalize' }
        ];
        ...
        contextMenuData.forEach(item => {
            ...
            if (item.name == 'New') {
                const contextMenuHr = document.createElement('hr');
                contextMenuHr.classList.add('context-menu-hr');
                contextMenuBox.appendChild(contextMenuHr);
            }
